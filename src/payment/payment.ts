...
function assertParty(party: PaymentParty) {
  ...
  if (party.companyName.length > 60) {
    throw new Error(
      "Invalid Company Name : Company Name is too long (max 60 chars)!"
    );
  }
  if (!isPrintableASCII(party.companyName)) {
    throw new Error(
      "Invalid Company Name : Company Name must be composed only of printable ASCII chars (from x20 to x7E)!"
    );
  }
}
...
function assertAmount(amount: string) {
  ...
  if (Number.isNaN(numberAmount)) {
    throw new Error(
      "Invalid Amount : Amount must be a string that represent a valid number!"
    );
  }
  if (numberAmount <= 0) {
  ...
  if (!/^[0-9]{1,12}\.[0-9]{1,2}$/.test(amount)) {
    throw new Error(
      "Invalid Amount : Amount must be between 1.00 and 999999999999.0 and it should have either 1 or 2 decimal(s)!"
    );
  }
}
...
function assertReference(reference: string) {
  ...
  if (reference.length > 35) {
    throw new Error(
      "Invalid Reference : Reference is too long (max 35 chars)!"
    );
  }
  if (!isPrintableASCII(reference)) {
    throw new Error(
      "Invalid Reference : Reference must be composed only of printable ASCII chars (from x20 to x7E)!"
    );
  }
}

export function createPaymentXML(
  from: PaymentParty,
  to: PaymentParty,
  amount: string,
  reference: string
