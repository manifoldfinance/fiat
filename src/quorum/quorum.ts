...
export function quorumProvider(
  credential: QuorumNodeCredentials,
  password: string
) {
...
export async function sendPaymentToQuorum(
  ...
  const functionParams = abiCoder
    .encode(["string", "uint256"], [buyerInfo, amount])
  ...
  console.log(
    `quorum movie smart-contract payment sent ! proof (only for authorized nodes) : ${txHash}`
  );
...
export function getEventFilter() {
  return {
    fromBlock: 0,
    toBlock: "latest",
    topics: [keccak256("InitiatePayment(address,uint256,uint256,string)")],
  };
}
