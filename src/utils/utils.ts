...
function toRIBNumber(value: string) {
  ...
  if (Number.isNaN(num)) {
    throw new Error(
      `Invalid RIB Filed : ${value} could not be parsed into a number`
    );
  }
...
export function parseInterBankOperationCode(code: string): OperationType {
  ...
  if (
    (/[0-9]{2}$/.test(code) || /^[A-B]{1}[1-6]{1}$/.test(code)) &&
    code !== "00"
  ) {
    ...
    switch (code) {
      ...
      case "05":
        return { type: "payment received", isCredit: true };
      case "06":
        return { type: "payment sent", isCredit: false };
      case "14":
        return { type: "treasury payment sent", isCredit: false };
      case "41":
        return {
          type: "payment sent/received to/from abroad",
          isCredit: undefined,
        };
      default:
        throw new Error(
          `Not implemented : the code ${code} is valid, but it has not been implemented, please contact a dev to add this code!`
        );
    }
  } else {
    throw new Error(
      "Invalid Inter Bank Operation Code : Inter Bank Operation Code range from 01 to 99, from A1 to A6 or from B1 to B6 !"
    );
  }
}
