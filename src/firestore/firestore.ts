...
export async function retrieveMoviesQuorumInfoFromInvoice(invoiceId: string) {
  ...
  if (!invoiceData) {
    throw new Error(
      `Invoice Not Found : the invoice with id ${invoiceId} doesn't exist ! This is probably caused by incoherent data in the firestore.`
    );
  }
  const moviesQuorumInfoPromises = (invoiceData.titles as InvoiceTitleDetails[]).map(
    async (title) => {
      ...
      if (!movieData) {
        throw new Error(
          `Movie Not Found : the movie with id ${movieId} doesn't exist ! This means that invoice #${invoiceId} contains incoherent 'titles' data.`
        );
      }
      const ethContractAddress = movieData.quorum.address;
      const stakeholdersEthAddresses = movieData.quorum
        .stakeholderNodes as string[];

      const stakeholdersPrivateForPromises = stakeholdersEthAddresses.map(
        async (ethAddress) => {
          const nodeSnapshot = await db
            .collection("quorumNodes")
            .doc(ethAddress)
            .get();
          const nodeData = nodeSnapshot.data();
          if (!nodeData) {
            throw new Error(
              `Quorum Node Not Found : the quorum node with address ${ethAddress} doesn't exist ! This means that the 'quorum.stakeholderNodes' key of the movie ${movieId} contains incoherent data.`
            );
          }
          return nodeData.privateFor as string;
        }
      );
      const stakeholdersPrivateFor = await Promise.all(
        stakeholdersPrivateForPromises
      );
...
export async function setInvoiceStatusToPaid(
  invoiceId: string,
  txHashProofs: string[]
) {
  try {
    await db
      .collection("invoices")
      .doc(invoiceId)
      .update({ status: "paid", txHashProofs });
  } catch (error) {
...
export async function persistUnmatchedPayment(payment: {
  account: string;
  movement: Movement;
}) {
  try {
...
export async function retrieveBankAccountFromEthAddress(
  ethAddress: string
  ...
  if (!nodeData) {
    throw new Error(
      `Quorum Node Not Found : There is no node with address ${ethAddress}!`
    );
  }
  ...
  if (!orgData) {
    throw new Error(
      `Organization Not Found : There is no org with id ${orgId}!`
    );
  }

  return {
    companyName: orgData.account.name,
    bic: orgData.account.BIC,
    iban: orgData.account.IBAN,
  };
}
