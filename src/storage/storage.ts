...
export async function clearTemporaryStorage() {
  console.warn(
    "This function has been disabled to prevent deleting potentially needed files!"
  );
  return;
  try {
    const fileNames = await readdirAsync(receiptFolder);
    await fileNames.forEach(
      async (name) => await unlinkAsync(`${receiptFolder}/${name}`)
    );
  } catch (error) {
    // no folder
  }
  ...
  try {
    ...
    await fileNames.forEach(
      async (name) => await unlinkAsync(`${paymentFolder}/${name}`)
    );
  } catch (error) {
    // no folder
  }
